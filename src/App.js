...
class PhoneBook extends Component {
          ...
          <tbody>
            <tr>
              ...
              <td>
                ...
                {this.state.editmode && (
                  <button onClick={this.savePhone}>Сохранить</button>
                )}
              </td>
              <td>
  ...
  deletePhone(id) {
    this.setState((state) => ({
      phones: this.state.phones.filter((phone) => phone.id !== id),
    }));
  }
...
// дополнительный компонент - список телефонной книги
      ...
      <tbody>
        {props.phones.map((phone) => (
          <tr key={phone.id}>
            ...
            <td>{phone.name}</td>
            <td>
              <button onClick={() => props.deletePhone(phone.id)}> x</button>
            </td>
          </tr>
        ))}
      </tbody>
